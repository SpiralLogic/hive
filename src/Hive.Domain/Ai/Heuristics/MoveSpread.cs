...
internal class MoveSpread : IHeuristic
{
    public int Get(HeuristicValues values, Move move)
    {
        ...
        var fromOpponentQueen =
            values.MoveFromLocation != null && values.MoveFromNeighbours.Exists(c => c.HasPlayerQueen(values.OpponentId));
        if (fromOpponentQueen && toOpponentQueen ||
            toOpponentQueen && (values.OpponentQueenNeighbours < 4 || values.MoveNeighbours.Count < 3))
