...
namespace Hive.Domain.Extensions
{
    internal static class Extensions
    {
        ...
        {
            return cells.SelectNeighbors(originCell)
                .WhereEmpty();
        }

        internal static IEnumerable<Cell> SelectNeighbors(this IEnumerable<Cell> cells, Cell originCell)
        {
            return cells.Intersect(originCell.Coords.GetNeighbors()
                .ToCells());
        }
        ...
        {
            return cells.WhereOccupied()
                .Where(c => c.Tiles.Any(t => t.PlayerId == playerId));
        }

        internal static ISet<Coords> ToCoords(this IEnumerable<Cell> cells)
        {
            return cells.SelectCoords()
                .ToHashSet();
        }
