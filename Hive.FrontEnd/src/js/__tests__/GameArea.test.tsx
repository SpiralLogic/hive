...
beforeEach(() => {
    ...
    jest.spyOn(CTX, 'useGameContext').mockImplementation(() => ({ moveTile: jest.fn(), ...gameState }));
});
...
test('default on drop is prevented', async () => {
    await TestUtils.act(async () => {
        ...
        render(<HiveContext.Provider value={CTX.useGameContext()}><GameArea loading={false}/></HiveContext.Provider>, container);
    });
